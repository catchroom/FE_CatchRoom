 ...
 * @returns { isBtnLoading, btnHandler } - isLoading는 버튼의 상태값, btnHandler는 버튼 핸들러 함수입니다.
 * @example const { isLoading, btnHandler } = useBtnLoading('/catch-items');
 */

export const useBtnLoading = (pushURL: string) => {
  const router = useRouter();
  const [isBtnLoading, setIsLoading] = React.useState<boolean>(false);
  ...
  return { isBtnLoading, btnHandler };
};
